...
export class WebGlCtxTexture extends CoreContextTexture {
  ...
  async onLoadRequest(): Promise<Dimensions> {
    ...
    if (
      ...
      //   createImageBitmap itself.  We use textureData.alreadyPremultiplied
      ...
      if (isImageBitmap) {
        const alreadyPremultiplied = textureData.alreadyPremultiplied ?? true;
        unpackPremultiply = alreadyPremultiplied
          ? false
