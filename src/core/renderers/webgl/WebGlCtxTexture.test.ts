...
// TextureData.alreadyPremultiplied records whether the platform did this:
//
...
// ---------------------------------------------------------------------------
// ImageBitmap — UNPACK selection driven by alreadyPremultiplied flag
// ---------------------------------------------------------------------------
describe('WebGlCtxTexture.onLoadRequest — UNPACK for ImageBitmap (alreadyPremultiplied flag)', () => {
  ...
  it(
    'UNPACK = false when alreadyPremultiplied is true ' +
  ...
  it(
    'UNPACK = true when alreadyPremultiplied is false and premultiplyAlpha is true ' +
  ...
  it(
    'UNPACK = false when alreadyPremultiplied is false and premultiplyAlpha is false ' +
  ...
  it(
    'UNPACK = false when alreadyPremultiplied is undefined ' +
      '(legacy / missing flag: backwards-compatible default)',
    async () => {
      // An ImageBitmap response that predates the alreadyPremultiplied flag.
      // Should behave as before (assume bitmap is already premultiplied).
      const bitmap = new (globalThis as any).ImageBitmap();
      const { instance, pixelStoreiSpy } = makeCtxTextureForUnpackTest({
        data: bitmap,
        premultiplyAlpha: true,
        // alreadyPremultiplied intentionally absent
      });
